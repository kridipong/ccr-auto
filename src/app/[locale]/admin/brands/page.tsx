...
export default function AdminBrands() {
  ...
  const handleSave = async () => {
    ...
    if (logoFile) {
      const { data } = await supabase.storage.from('brand-logos').upload(
        `logos/${Date.now()}-${logoFile.name}`, logoFile
      )
      if (data) {
        logo_url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/brand-logos/${data.path}`
      }
    }
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <label className="inline-flex items-center gap-2 px-4 py-2 border rounded-lg cursor-pointer text-sm text-gray-600 hover:bg-gray-50">
                  <Upload size={16} /> {locale === 'th' ? 'อัปโหลดโลโก้' : 'Upload Logo'}
